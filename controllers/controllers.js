...
asociacionController.update = async function (req, res, next){
    ...
    try{
        ...
        res.status(200).json({message: "asociacion actualizada correctamente"});
    }
    catch(err){
        return res.status(500).json({err: err, message: "no has insertado correctamente los datos"});
    }
}
...
asociacionController.create = async function(req,res, next){
    ...
    try{
        await asoc.save();
        return res.status(200).json({message: "se guardo con exito"})
    }
    catch(err){
        return res.status(500).json({err: err, message: "no se pudo guardar, revisa los campos"});
    }
}
...
asociacionController.delete = async function(req, res, next){
    ...
    res.status(200).json({message: "asociacion borrada con exito"});
}
